# this file trains a Stock Selection Agent.
...
#long_agent = once_long_agent(path_to_save,gamma,max_mem_size,C,batch_size,epsilon, eps_min, eps_dec, num_layers,hidden_size,added_size,fc1_dims,fc2_dims,lr,wd,lookback,x2_size,scheduler_max_value,scheduler_initial)
...
while end <= last:
    ...
    env_special = Enviroment(start = start, end  = end,lookback = lookback,cluster = c, cash = 1e6, p_finder = shares)
    ...
    for day in range(env_special.start,env_special.end):
        ...
        for n,stock in enumerate(stocks):
            ...
            running_count = running_counts[n]
        ...
        env_special.step([i for i in running_counts])

    # stop finding training policy 
                  
    
    long_agent.epsilon = epsilon/100
