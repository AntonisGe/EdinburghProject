...
def long(env,action,running_count,list_of_buy_days):
    '''
    ...
    if action == 0:
        ...
        if running_count ==0:
            ...
            for buy_day in list_of_buy_days:
                ...
                print(f'You bougght at {price_bought}, sold at {price_sold} with tcost {t_cost}. Your profit was {price_sold - price_bought - t_cost}.')
...
def short(env,action,running_count,list_of_buy_days):
    '''
    output: reward,running_count,list_of_buy_days
    '''
    if action == 0:
        ...
        if running_count ==0:
            ...
            for buy_day in list_of_buy_days:
                ...
                print(f'You short sold at {price_sold}, and bought back in {price_bought} with tcost {t_cost}. Your profit was {price_sold - price_bought - t_cost}.')
...
for day in range(env.start,env.end):
    ...
    #print(f'Paper 1 data is {env.paper_1_data(stock)}')
    print(f'Paper 1 data is {env.SSA_matrix(stock)}')
    ...
    long_agent.store_transition(x1,x2,action,reward,x1_next,x2_next,done,0)
