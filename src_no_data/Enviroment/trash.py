...
class Enviroment:
    ...
    def step(self,w: list):
        ...
        # ADD CODE ABOVE HERE 
        self.timestep += 1
        value_of_new_port = self.value_of_port(new_por,write = True)
        self.record.store(self.timestep,new_por,tcosts,value_of_new_port,cash_left,self.fake_cash)    
    ...
    def value_of_port(self,new_por,write = False):
        new_por1 = [abs(i) for i in new_por]
               
        total_value_fale_cash = self.fake_cash
        if sum(new_por1) != 0:
            old_port = self.portofolio
        
            for new,old,stock_no in zip(new_por,old_port,range(len(new_por))):
                if new<0:
                    diff = new - min(old,0)
                    new_fake_cash = -2*diff*self.closes[stock_no,self.timestep]
                    if write:
                        self.fake_cash += new_fake_cash
                        self.fake_cashses[stock_no] += new_fake_cash
                        
                else:
                    if old<0:
                        new_fale_cash = self.fake_cashses[stock_no]
                        if write:
                            self.fake_cashses[stock_no] = 0
                            self.fake_cash -= new_fake_cash
                    else:
                        new_fake_cash = 0
            
            total_value_fale_cash += new_fake_cash
        ...
        return value + total_value_fale_cash
    ...
    def reset(self):  
        ...
        self.fake_cash = 0
        self.fake_cashses = [0 for _ in range(self.num_stocks)]
...
class Record:
    ...
    def store(self,timestep,portofolio,tcosts,value_of_new_port,cash_left,fake_cash):
        ...
        self.valuation[timestep] = cash_left + value_of_new_port #+ fake_cash
...
if __name__ == '__main__':
    ...
    env = Enviroment(stocks = ['AAPL','BA'], start = start, end = end, transaction_cost=0.5,variance = 0,p_finder=shares,lookback = 20)
